...
export class StatblockEditComponent implements OnInit, OnDestroy {
    ...
    return new Promise((resolve, reject) => {
      this.statblockService.getStatblocks().subscribe({
        next: (statblocks) => {
          ...
          this.processStatblocksInChunks(sortedStatblocks)
            .then(() => {
  ...
  private async processStatblocksInChunks(
  ...
  scrollToStatblock(statblockId: string): void {
    ...
    const targetRowIndex = this.editableRows.findIndex(
      (row) => row.id === statblockId,
    );
      ...
      filtered = this.allStatblocks.filter((statblock) => {
        return searchTerms.every((term) => {
          const searchableText =
            `${statblock.name} ${statblock.tags?.join(" ") || ""}`.toLowerCase();
  ...
  toggleTagFilter(tag: string): void {
    ...
    const tagIndex = searchTerms.findIndex(
      (term) => term.toLowerCase() === tag.toLowerCase(),
    );
  ...
  onResistancesChange(row: EditableStatBlock): void {
    this.debouncedChange(row, () => {
      ...
      const lines = row.resistancesText
        .split("\n")
        .filter((line) => line.trim());
  ...
  private async handleSidebarImage(
    source: File | { url: string } | { base64: string },
  ): Promise<void> {
    try {
      ...
      const created = await firstValueFrom(
        this.statblockService.createStatblock(defaults),
      );
      // 2) Insert into lists
      ...
      if (row.id) {
        if (source instanceof File) {
          await firstValueFrom(
            this.statblockService.uploadImage(row.id, source),
          );
        } else if ("url" in source) {
          await firstValueFrom(
            this.statblockService.setImageFromUrl(row.id, source.url),
          );
        } else if ("base64" in source) {
          await firstValueFrom(
            this.statblockService.setImageFromBase64(row.id, source.base64),
          );
        }
        row.hasImage = true;
  ...
  openPasteDialog(): void {
    ...
    ref
      .afterClosed()
      .subscribe(
        (result: File | { url: string } | { base64: string } | undefined) => {
          if (!result) return;
  ...
  deleteSelected(): void {
    ...
    if (confirm(`Delete ${selectedRows.length} selected statblocks?`)) {
      const toDelete: EditableStatBlock[] = selectedRows.filter(
        (row: EditableStatBlock) => !row.isNew && !!row.id,
      );
      const newRows: EditableStatBlock[] = selectedRows.filter(
        (row: EditableStatBlock) => row.isNew,
      );
  ...
  isAllSelected(): boolean {
    return (
      ...
      this.selection.selected.length === this.editableRows.length
    );
  }
  ...
  copyRow(source: EditableStatBlock): void {
    ...
    const copy: EditableStatBlock = {
      ...
      attacksText:
        source.attacksText ||
        source.attacks?.map((a) => a.name).join("\n") ||
        "",
      spellsText:
        source.spellsText || source.spells?.map((s) => s.name).join("\n") || "",
      spellSlotsText:
        source.spellSlotsText || source.spellSlots?.join(" ") || "",
      skillsText: source.skillsText || source.skills?.join("\n") || "",
      resistancesText:
        source.resistancesText || source.resistances?.join("\n") || "",
    ...
    const insertIndex =
      masterIndex >= 0 ? masterIndex + 1 : this.allStatblocks.length;
