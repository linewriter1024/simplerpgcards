  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @ViewChildren("slicesContainer") private slicesContainers!: QueryList<
    ElementRef<HTMLDivElement>
  >;
  private containerWidths: Record<string, number> = {};
  private layoutById: Record<
    string,
    { tileWidth: number; scale: number; chunkImgPx: number; columns: number }
  > = {};
  ...
  ngAfterViewInit(): void {
    ...
    this.slicesContainers.changes.subscribe(() =>
      this.scheduleMeasureCompute(),
    );
  ...
  applyFilters(): void {
    ...
    if (tokens.length > 0) {
      filtered = filtered.filter((statblock) =>
        this.matchesAllTokens(statblock, tokens),
      );
    }
  ...
  private parseSearchTerms(
    searchText: string,
  ): Array<{ value: string; exact: boolean }> {
  ...
  private tokensToSearchText(
    tokens: Array<{ value: string; exact: boolean }>,
  ): string {
  ...
  toggleActiveOnly(): void {
    ...
    const activeTokenIndex = tokens.findIndex(
      (t) => t.value.toLowerCase() === "active" && t.exact,
    );
  ...
  toggleContextOnly(): void {
    ...
    const contextTokenIndex = tokens.findIndex(
      (t) => t.value.toLowerCase() === this.contextTag.toLowerCase() && t.exact,
    );
  ...
  addActiveToSelected(): void {
    ...
    selectedStatblocks.forEach((statblock) => {
      if (!this.isStatblockActive(statblock)) {
        ...
        updates.push(
          this.statblockService
            .updateStatblock(statblock.id!, updateData)
            .toPromise(),
        );
      }
    });
  ...
  removeActiveFromSelected(): void {
    ...
    selectedStatblocks.forEach((statblock) => {
      if (this.isStatblockActive(statblock)) {
        const updatedTags = (statblock.tags || []).filter(
          (tag) => tag !== "active",
        );
        const updateData = {
          name: statblock.name,
  ...
  setContextTag(newTag: string): void {
    ...
    if (trimmedTag.includes(" ")) {
      alert(
        "Context tag cannot contain spaces. Please use underscores or hyphens instead.",
      );
      return;
    }
    ...
    const currentSearch = this.searchControl.value || "";
    const tokens = this.parseSearchTerms(currentSearch);
    const hasOldContextTag = tokens.some(
      (t) => t.value.toLowerCase() === this.contextTag.toLowerCase() && t.exact,
    );
  ...
  toggleAllRows(): void {
    if (this.isAllSelected()) {
      this.filteredStatblocks.forEach((statblock) =>
        this.selection.deselect(statblock),
      );
    } else {
  ...
  addTagToSelected(): void {
    ...
    if (tagToAdd.includes(" ")) {
      alert(
        "Tags cannot contain spaces. Please use underscores or hyphens instead.",
      );
      return;
    }
  ...
  isStatblockEmpty(sb: StatBlock): boolean {
    const textEmpty = !(
      (sb.type && sb.type.trim()) ||
      (sb.cr && sb.cr.trim()) ||
      (sb.hp && String(sb.hp).trim()) ||
      (sb.ac && String(sb.ac).trim()) ||
      (sb.notes && sb.notes.trim())
    );
    const arraysEmpty =
  ...
  getSliceStyle(sb: StatBlock, index: number): { [k: string]: any } {
    ...
    const heightCss = Math.max(
      0,
      Math.min(
        Math.round(this.getChunkHeightFor(id) * s),
        Math.floor(effectiveScale * ((dims?.height || 0) - topImgPx)),
      ),
    );
    return {
  ...
  adjustSliceOffset(sb: StatBlock, delta: number): void {
    ...
    // Persist offset
    this.statblockService
      .updateImageSettings(sb.id, { offset: next })
  ...
  adjustImageScale(sb: StatBlock, dir: "up" | "down"): void {
    ...
    // Persist scale
    this.statblockService
      .updateImageSettings(id, { scale: next })
      .subscribe({ next: () => {}, error: () => {} });
  }
  ...
  resetImageSettings(sb: StatBlock): void {
    ...
    this.scheduleMeasureCompute();
    this.statblockService
      .updateImageSettings(id, { offset: 0, scale: 1 })
      .subscribe({
        next: (s) => {
  ...
  private computeLayoutForId(id: string | undefined): void {
    ...
    const maxColsByWidth = Math.max(
      ...
      Math.min(
        maxColsAbsolute,
        Math.floor((containerW + gap) / (minTileWidth + gap + borderX)),
      ),
    );
    ...
    const tryCols = (cols: number) => {
      const availableW = Math.max(
        0,
        containerW - (cols - 1) * gap - cols * borderX,
      );
      ...
      const chunkImgPx = Math.max(
        1,
        Math.min(maxReasonableChunk, Math.ceil(ch / scale)),
      );
      const remaining = Math.max(0, dims.height - offset);
    ...
    for (let cols = maxColsByWidth; cols >= 1; cols--) {
      ...
      if (res.tw >= minTileWidth && res.rows <= this.maxRows) {
        if (
          !best ||
          res.effCols > best.effCols ||
          (res.effCols === best.effCols && res.tw > best.tw)
        ) {
          best = res;
    ...
    if (scaleBias !== 1) {
      const desiredTw = Math.max(
        minTileWidth,
        Math.min(dims.width, Math.round(final.tw * scaleBias)),
      );
